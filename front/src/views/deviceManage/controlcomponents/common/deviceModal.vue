 ...
 * @LastEditTime: 2021-02-05 13:56:40
    ...
    >
      ...
      <a-spin :spinning="spinning">
        <div class="modal-border-panel mt20">
          <a-radio-group v-model="checkedList">
...
export default {
  ...
  watch: {
    data(newVal, oldVal) {
      if (newVal && newVal[0]) {
        this.did = newVal[0];
        this.getDevice().then((resp) => {
          for (var item of resp) {
            if (item.did == newVal[0]) {
  ...
  methods: {
    ...
    changeCheckedList(e) {
      this.checkedList = [];
      // console.log("changeCheckedList",e)
      // if (e.target.checked){
    ...
    ok() {
      ...
      const params = [];
      const index = deviceList.findIndex((k) => k.id === checkedList);
      if (index > -1) {
    ...
    getDevice() {
      return new Promise((resolve, reject) => {
        this.spinning = true;
        const { search, flag, id, remark, productid, groupid, did } = this;
        const params = {
